 ...
 * @tpTestCaseDetails Test for hostnameVerifier
 * @tpSince RESTEasy 3.6.3.Final
 */
@RunWith(Arquillian.class)
@RunAsClient
public class CustomHostnameVerifierTest extends SSLTestBase {

   private static final Logger LOG = Logger.getLogger(CustomHostnameVerifierTest.class.getName());
    ...
    * Server/endpoint is secured with the same self-signed certificate and server actual host address is not included among 'subject alternative names' in the certificate.
    * Instead it was generated for random address: 199.111.68.7.
    * Client should trust the server because custom HostnameVerifier is configured to return true for server actual host address (HOSTNAME).
    * @tpSince RESTEasy 3.6.3.Final
    */
   @Test
   public void test() {
