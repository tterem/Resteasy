...
public class DynamicFeatureTest {
    ...
    * - dynamic feature actually works by checking whether interceptor was used
    * @tpSince RESTEasy 3.8.0
    */
   @Test
   public void testDynamicFeature() {
      ...
      if (counter == 0) {
         Assert.fail("Dynamic features were not processed at application deployment time");
      }

      // TODO uncomment following condition once https://issues.jboss.org/browse/RESTEASY-2246 is resolved
//      if (counter > 1) {
//         Assert.fail("Dynamic features for filters and interceptors should be resolved only once for each resource method but were resolved " + counter + " times.");
//      }
