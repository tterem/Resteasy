 ...
 * @tpTestCaseDetails Test for HostnameVerificationPolicy
 * @tpSince RESTEasy 3.6.3.Final
 */
@RunWith(Arquillian.class)
@RunAsClient
public class HostnameVerificationPolicyStrictNokTest {

   protected static final Logger LOG = Logger.getLogger(HostnameVerificationPolicyStrictNokTest.class.getName());
   ...
   private static String SERVER_KEYSTORE_PATH = RESOURCES + "server2.keystore";
   private static final String CLIENT_TRUSTSTORE_PATH = RESOURCES + "client2.truststore";
   ...
   public static Archive<?> deployDefault() throws Exception {
      ...
      war.addClass(HostnameVerificationPolicyResource.class);
   ...
   public void test() {
      ...
      client.target("https://" + PortProviderUtil.getHost() + ":8443/war_default/secure/hello").request().get();
   }
   ...
   private static void secureServer() throws Exception {
      ...
      TestUtil.runCmd(client, String.format("/core-service=management/security-realm=ApplicationRealm/server-identity=ssl:add(keystore-path=%s, keystore-password=%s, alias=\"cn=server\")", SERVER_KEYSTORE_PATH, PASSWORD));
